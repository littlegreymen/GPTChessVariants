...
class Game:
    ...
    def evaluate_board(self, board_sim) -> float:
        score = board_sim.material_score()  # already signed so positive favors black
        try:
            black_moves = len(board_sim.all_legal_moves("black"))
            white_moves = len(board_sim.all_legal_moves("white"))
            mobility = black_moves - white_moves
        except Exception:
            mobility = 0
            score += 0.12 * mobility  # tuneable weight
    ...
    def _order_moves(self, board_sim, moves, for_color):
        ...
        for f,t in moves:
            ...
            if target:
                values = { "Pawn":1, "Knight":3, "Bishop":3, "Rook":5, "Queen":9, "King":0 }
                score += values.get(type(target).__name__, 0) * 10
            # promotion heuristic: if pawn moves to last rank (we can detect by row)
            piece = board_sim.get_piece(f)
            if piece and isinstance(piece, type(piece)) and type(piece).__name__ == "Pawn":
                if (piece.color=="black" and t[0]==7) or (piece.color=="white" and t[0]==0):
                    score += 50
    ...
    def _minimax(self, board_sim, depth:int, alpha:float, beta:float, maximizing:bool) -> float:
        if depth == 0:
            return self.evaluate_board(board_sim)

        color = "black" if maximizing else "white"
        moves = board_sim.all_legal_moves(color)
        if not moves:
            # No legal moves: check for checkmate/stalemate
            if board_sim.is_in_check(color):
                # losing position for color to move. If maximizing (black) to move and no moves -> black is checkmated => very bad
                return -9999 if maximizing else 9999
            return 0  # stalemate

        # order moves for alpha-beta efficiency (we order on the *current* board)
    ...
    def ai_move(self):
        ...
        best_score = -99999.0
        best_moves = []
        # Root search: for each candidate move do one ply (simulate) then minimax depth-2
        for f,t in moves:
            sim = self.board.clone()
            sim.move_piece(f,t)
            val = self._minimax(sim, depth=2, alpha=-99999.0, beta=99999.0, maximizing=False)
