...
# --------------------------
# Optional ggwave sound transport
# --------------------------
class AirwaveTransport:
    SAMPLE_RATE = 48000
    RX_CHUNK = 1024
    TX_CHUNK = 4096
    PROTOCOL_ID = 1
    ...
    @staticmethod
    def is_available():
        return ggwave is not None and pyaudio is not None

    @staticmethod
    def missing_reason():
        missing = []
        if ggwave is None:
            missing.append("ggwave")
        if pyaudio is None:
    ...
    def start(self):
        if not self.is_available():
            self.on_status(self.missing_reason())
            return False
        ...
        self.on_status("Sound listener started.")
        return True
    ...
    def _send_worker(self, text):
        ...
        try:
            ...
            waveform = ggwave.encode(text, protocolId=self.PROTOCOL_ID, volume=self.VOLUME)
            stream = audio.open(
                format=pyaudio.paFloat32,
                channels=1,
                rate=self.SAMPLE_RATE,
                output=True,
                frames_per_buffer=self.TX_CHUNK,
            )
            stream.write(waveform, len(waveform) // 4)
            self.on_status("Sound message transmitted.")
        except Exception as exc:
            self.on_status(f"Sound transmit failed: {exc}")
        finally:
    ...
    def _listen_loop(self):
        ...
        try:
            ...
            stream = audio.open(
                ...
                rate=self.SAMPLE_RATE,
                input=True,
                frames_per_buffer=self.RX_CHUNK,
            )
            instance = ggwave.init()
            while not self.stop_event.is_set():
                data = stream.read(self.RX_CHUNK, exception_on_overflow=False)
                result = ggwave.decode(instance, data)
                if result is None:
                    continue
                try:
                    self.on_message(result.decode("utf-8"))
                except UnicodeDecodeError:
                    self.on_status("Ignored undecodable sound message.")
        except Exception as exc:
            self.on_status(f"Sound listener stopped: {exc}")
        finally:
...
class Game:
    ...
    AI_USE_MULTIPROCESS = True
    AI_PARALLEL_MIN_MOVES = 2
    AI_PARALLEL_MAX_WORKERS = None
    ...
    def __init__(self):
        ...
        self._ai_executor_workers = 0
        self._last_ai_worker_count = 1

    def _parallel_worker_count(self, move_count):
        if not self.AI_USE_MULTIPROCESS or move_count < self.AI_PARALLEL_MIN_MOVES:
            return 1
        cpu_count = os.cpu_count() or 1
        configured_workers = self.AI_PARALLEL_MAX_WORKERS or cpu_count
        return max(1, min(move_count, cpu_count, configured_workers))

    def _get_ai_executor(self, worker_count):
        if worker_count <= 1:
            return None
        if self._ai_executor is None or self._ai_executor_workers != worker_count:
            self.shutdown_ai_executor(wait=False)
            self._ai_executor = ProcessPoolExecutor(max_workers=worker_count)
            self._ai_executor_workers = worker_count
        return self._ai_executor

    def shutdown_ai_executor(self, wait=True):
        if self._ai_executor is not None:
            try:
                self._ai_executor.shutdown(wait=wait, cancel_futures=True)
            except TypeError:
                self._ai_executor.shutdown(wait=wait)
            self._ai_executor = None
            self._ai_executor_workers = 0
    ...
    def _score_black_root_move_serial(self, move):
        f, t = move
    ...
    def _score_black_root_moves_serial(self, ordered_moves):
        self._search_cache = {}
        self._last_ai_worker_count = 1
        return [self._score_black_root_move_serial(move) for move in ordered_moves]

    def _score_black_root_moves_parallel(self, ordered_moves):
        worker_count = self._parallel_worker_count(len(ordered_moves))
        if worker_count <= 1:
            return None

        board_snapshot = self.board.clone()
        payloads = [
            (board_snapshot, move, self.AI_SEARCH_DEPTH, self.AI_QUIESCENCE_DEPTH)
            for move in ordered_moves
        ]
        executor = self._get_ai_executor(worker_count)
        self._last_ai_worker_count = worker_count
        return list(executor.map(_score_black_root_move_worker, payloads))

    def _score_black_root_moves(self, ordered_moves):
        try:
            parallel_scores = self._score_black_root_moves_parallel(ordered_moves)
            if parallel_scores is not None:
                return parallel_scores
        except Exception as exc:
            print(f"Parallel AI search failed; falling back to serial ({exc}).")
            self.shutdown_ai_executor(wait=False)

        return self._score_black_root_moves_serial(ordered_moves)
    ...
    def _choose_ai_move(self):
        ...
        scored_moves = self._score_black_root_moves(ordered)
...
def _score_black_root_move_worker(payload):
    board_snapshot, move, search_depth, quiescence_depth = payload
    game = Game()
    game.AI_USE_MULTIPROCESS = False
    game.AI_SEARCH_DEPTH = search_depth
    game.AI_QUIESCENCE_DEPTH = quiescence_depth
    game.board = board_snapshot.clone()
    game._search_cache = {}
    return game._score_black_root_move_serial(move)
...
class DPGChessGame(Game):
    ...
    def __init__(self):
        ...
        self.game_id = None
        self.sound_move_no = 0
        self.last_ai_time_seconds = None
        self.airwave_transport = None
        self.airwave_inbox = queue.Queue()
        self.airwave_status = queue.Queue()
        self.log_lines = []
        self.window_tag = "chessvultra_dpg_window"
        self.mode_modal_tag = "chessvultra_mode_modal"
        self.status_tag = "chessvultra_dpg_status"
        self.ai_time_tag = "chessvultra_dpg_ai_time"
        self.sound_status_tag = "chessvultra_dpg_sound_status"
    ...
    def _set_status(self, message):
        if dpg is not None and dpg.does_item_exist(self.status_tag):
    ...
    def _sync_ai_time(self):
        if dpg is not None and dpg.does_item_exist(self.ai_time_tag):
            dpg.set_value(self.ai_time_tag, self._format_ai_time(self.last_ai_time_seconds))

    def _set_sound_status(self, message):
        if dpg is not None and dpg.does_item_exist(self.sound_status_tag):
            dpg.set_value(self.sound_status_tag, message)

    def _opponent_color(self, color):
        return "black" if color == "white" else "white"

    def _local_player_color(self):
        if self.game_mode == self.MODE_AI:
            return "white"
        if self.game_mode == self.MODE_SOUND:
    ...
    def _board_sync_hash(self):
        payload = repr(self._board_signature(self.board)).encode("utf-8")
        return hashlib.sha1(payload).hexdigest()[:8]

    def _encode_airwave_message(self, kind, *parts):
        return "|".join(["CVU1", kind, *[str(part) for part in parts]])

    def _queue_airwave_message(self, text):
        self.airwave_inbox.put(text)

    def _queue_airwave_status(self, message):
        self.airwave_status.put(message)

    def _send_airwave_text(self, text):
        if self.airwave_transport is None:
            self._set_sound_status("Sound transport is not running.")
            return False
        return self.airwave_transport.send_text(text)

    def _send_airwave_hello(self, sender=None, app_data=None, user_data=None):
        if self.game_mode != self.MODE_SOUND:
            self._set_status("Sound pairing is only used in human sound mode.")
            return
        if not self.game_id:
            self.game_id = self._new_game_id()
        message = self._encode_airwave_message("HELLO", self.game_id, self.local_color or "unknown")
        if self._send_airwave_text(message):
            self._log(f"Sent sound pairing for game {self.game_id}.")

    def _send_airwave_reset(self):
        if self.game_mode == self.MODE_SOUND and self.game_id:
            self._send_airwave_text(self._encode_airwave_message("RESET", self.game_id))

    def _send_airwave_move(self, from_pos, to_pos):
        if self.game_mode != self.MODE_SOUND or not self.game_id:
            return
        move_text = coord_to_notation(from_pos) + coord_to_notation(to_pos)
        message = self._encode_airwave_message(
            "MOVE",
            self.game_id,
            self.sound_move_no,
            move_text,
            self._board_sync_hash(),
        )
        if self._send_airwave_text(message):
            self._log(f"Sent move over sound: {move_text}.")
    ...
    def _sync_log(self):
        if dpg is not None and dpg.does_item_exist(self.log_tag):
    ...
    def _snapshot_state(self):
        return {
            ...
            "sound_move_no": self.sound_move_no,
    ...
    def _restore_state(self, state):
        ...
        self.sound_move_no = state.get("sound_move_no", self.sound_move_no)
    ...
    def _update_status_for_turn(self):
        if self.game_mode is None:
            self._set_status("Choose a game mode to start.")
            return False
        ...
        check_note = ""
        if self.board.is_in_check(self.turn):
            check_note = f"{self.turn.capitalize()} is in check. "

        if self.game_mode == self.MODE_AI:
            if self.turn == "white":
                self._set_status(check_note + "White to move. Click a source square, then a target square.")
            else:
                self._set_status(check_note + "Black to move.")
        elif self.turn == self.local_color:
            self._set_status(check_note + f"Your move as {self.local_color.capitalize()}.")
        else:
            self._set_status(check_note + f"Waiting for {self.turn.capitalize()} over sound.")
        return False
    ...
    def _make_ai_reply(self):
        ...
        self._sync_ai_time()
        worker_note = "serial" if self._last_ai_worker_count <= 1 else f"{self._last_ai_worker_count} processes"
        self._log(f"Black AI took {elapsed:.3f} seconds ({worker_note}).")
        self.turn = "white"
        self._reset_selection()
        self._update_status_for_turn()
        self.refresh_board()

    def _start_airwave_transport(self):
        if self.airwave_transport is not None:
            self.airwave_transport.stop()
        self.airwave_transport = AirwaveTransport(self._queue_airwave_message, self._queue_airwave_status)
        if not self.airwave_transport.start():
            self._set_sound_status(AirwaveTransport.missing_reason())
            self._log("Sound mode selected, but ggwave/pyaudio is not available.")
            return False
        return True

    def _stop_airwave_transport(self):
        if self.airwave_transport is not None:
            self.airwave_transport.stop()
            self.airwave_transport = None

    def _hide_mode_modal(self):
        if dpg is not None and dpg.does_item_exist(self.mode_modal_tag):
            dpg.configure_item(self.mode_modal_tag, show=False)

    def _show_mode_modal(self, sender=None, app_data=None, user_data=None):
        if dpg is not None and dpg.does_item_exist(self.mode_modal_tag):
            dpg.configure_item(self.mode_modal_tag, show=True)

    def _select_ai_mode(self, sender=None, app_data=None, user_data=None):
        self._stop_airwave_transport()
        self.game_mode = self.MODE_AI
        self.local_color = "white"
        self.game_id = None
        self._hide_mode_modal()
        self.new_game(send_reset=False)

    def _select_sound_mode(self, color):
        self.shutdown_ai_executor(wait=False)
        self.game_mode = self.MODE_SOUND
        self.local_color = color
        self.game_id = self._new_game_id() if color == "white" else None
        self._hide_mode_modal()
        self.new_game(send_reset=False)
        if self._start_airwave_transport() and color == "white":
            self._send_airwave_hello()

    def _select_sound_white(self, sender=None, app_data=None, user_data=None):
        self._select_sound_mode("white")

    def _select_sound_black(self, sender=None, app_data=None, user_data=None):
        self._select_sound_mode("black")

    def _poll_airwave_queues(self):
        while True:
            try:
                message = self.airwave_status.get_nowait()
            except queue.Empty:
                break
            self._set_sound_status(message)
            if message.startswith(("Missing", "Sound listener stopped", "Sound transmit failed")):
                self._log(message)

        while True:
            try:
                text = self.airwave_inbox.get_nowait()
            except queue.Empty:
                break
            self._handle_airwave_text(text)

    def _handle_airwave_text(self, text):
        parts = text.strip().split("|")
        if len(parts) < 2 or parts[0] != "CVU1":
            return
        if self.game_mode != self.MODE_SOUND:
            return

        kind = parts[1]
        if kind == "HELLO" and len(parts) >= 4:
            self._handle_airwave_hello(parts[2], parts[3])
        elif kind == "MOVE" and len(parts) >= 6:
            self._handle_airwave_move(parts[2], parts[3], parts[4], parts[5])
        elif kind == "ACK" and len(parts) >= 4:
            self._handle_airwave_ack(parts[2], parts[3])
        elif kind == "RESET" and len(parts) >= 3:
            self._handle_airwave_reset(parts[2])

    def _handle_airwave_hello(self, remote_game_id, remote_color):
        if self.local_color == remote_color:
            return
        if self.game_id is None:
            self.game_id = remote_game_id
            self._log(f"Paired sound game {self.game_id}.")
            self._set_sound_status(f"Paired game {self.game_id}.")
            self._send_airwave_text(self._encode_airwave_message("ACK", self.game_id, "HELLO"))
        elif self.game_id == remote_game_id:
            self._set_sound_status(f"Sound paired: {self.game_id}.")

    def _handle_airwave_ack(self, remote_game_id, ack_target):
        if self.game_id and remote_game_id == self.game_id:
            self._set_sound_status(f"Sound ACK received: {ack_target}.")

    def _handle_airwave_reset(self, remote_game_id):
        if self.game_id != remote_game_id:
            return
        self._log("Received remote new-game reset.")
        self.new_game(send_reset=False)

    def _handle_airwave_move(self, remote_game_id, move_no_text, move_text, expected_hash):
        if self.game_id is None and self.local_color == "black":
            self.game_id = remote_game_id
        if self.game_id != remote_game_id:
            return

        try:
            move_no = int(move_no_text)
        except ValueError:
            self._set_sound_status("Ignored sound move with bad sequence number.")
            return

        if move_no <= self.sound_move_no:
            self._send_airwave_text(self._encode_airwave_message("ACK", self.game_id, move_no))
            return
        if move_no != self.sound_move_no + 1:
            self._set_status(f"Sound move out of sequence: got {move_no}, expected {self.sound_move_no + 1}.")
            return
        if len(move_text) < 4:
            self._set_sound_status("Ignored malformed sound move.")
            return

        try:
            from_pos = notation_to_coord(move_text[:2])
            to_pos = notation_to_coord(move_text[2:4])
        except Exception:
            self._set_sound_status("Ignored sound move with bad notation.")
            return

        remote_color = self._opponent_color(self.local_color)
        piece = self.board.get_piece(from_pos)
        if self.turn != remote_color or piece is None or piece.color != remote_color:
            self._set_status("Ignored out-of-turn sound move.")
            return

        snapshot = self._snapshot_state()
        if not self._make_gui_move(from_pos, to_pos, validate=True):
            self._restore_state(snapshot)
            self._set_status("Ignored illegal sound move.")
            return

        self.sound_move_no = move_no
        actual_hash = self._board_sync_hash()
        if actual_hash != expected_hash:
            self._restore_state(snapshot)
            self._set_status("Sound move caused a board mismatch; move rejected.")
            self._log(f"Board hash mismatch: expected {expected_hash}, got {actual_hash}.")
            return

        self.turn = self.local_color
        self._reset_selection()
        self._log(f"Received move over sound: {move_text[:4]}.")
        self._send_airwave_text(self._encode_airwave_message("ACK", self.game_id, move_no))
        self._update_status_for_turn()
        self.refresh_board()
    ...
    def on_square_click(self, sender, app_data, user_data):
        if self.game_over:
            return
        local_color = self._local_player_color()
        if local_color is None:
            self._set_status("Choose a game mode first.")
            self._show_mode_modal()
            return
        if self.turn != local_color:
            if self.game_mode == self.MODE_AI:
                self._set_status("Black is thinking.")
            else:
                self._set_status(f"Waiting for {self.turn.capitalize()} over sound.")
            return
        ...
        if self.selected_square is None:
            if piece is None:
                self._set_status(f"Select a {local_color} piece first.")
                return
        ...
        if piece is not None and piece.color == local_color and pos not in self.legal_targets:
            self.selected_square = pos
            self.legal_targets = set(self.board.legal_moves_for(pos))
            self._set_status(f"Selected {coord_to_notation(pos)}. Click a target square.")
            self.refresh_board()
        ...
        if self._make_gui_move(from_pos, pos):
            ...
            self.turn = self._opponent_color(local_color)
            self._reset_selection()
            self.refresh_board()
            if self.game_mode == self.MODE_AI and not self._update_status_for_turn():
                self._make_ai_reply()
            elif self.game_mode == self.MODE_SOUND:
                self.sound_move_no += 1
                self._send_airwave_move(from_pos, pos)
                self._update_status_for_turn()

    def undo_white_move(self, sender=None, app_data=None, user_data=None):
        if self.game_mode == self.MODE_SOUND:
            self._set_status("Undo is disabled for sound games.")
            return
    ...
    def new_game(self, sender=None, app_data=None, user_data=None, send_reset=True):
        should_send_reset = send_reset and self.game_mode == self.MODE_SOUND and self.game_id is not None
        self.board = ChessBoard()
        self.turn = "white"
        self.last_move = None
        self.game_over = False
        ...
        if self.game_mode == self.MODE_AI:
            ...
            color_label = self.local_color.capitalize() if self.local_color else "Unknown"
            game_label = self.game_id or "waiting for host"
            self._log(f"Human sound game. You are {color_label}. Game ID: {game_label}.")
            if self.local_color == "white":
                self._log("Play White locally. Your moves are sent over sound.")
            else:
                self._log("Waiting for White's move over sound.")
        else:
            self._log("Choose a game mode to start.")
        self._sync_ai_time()
        if should_send_reset:
            self._send_airwave_reset()
        self._update_status_for_turn()
        self.refresh_board()
    ...
    def run(self):
        ...
        with dpg.window(label="ChessvUltra DPG", tag=self.window_tag):
            ...
            dpg.add_text("", tag=self.sound_status_tag)
            ...
            with dpg.group(horizontal=True):
                ...
                with dpg.group():
                    dpg.add_button(label="Choose Mode", width=self.SIDE_PANEL_WIDTH, callback=self._show_mode_modal)
                    ...
                    dpg.add_button(label="Pair/Resend Sound ID", width=self.SIDE_PANEL_WIDTH, callback=self._send_airwave_hello)
        ...
        with dpg.window(label="Choose Game Mode", tag=self.mode_modal_tag, modal=True, show=True, no_close=True, width=420, height=190):
            dpg.add_text("Choose how to play this game.")
            dpg.add_spacer(height=8)
            dpg.add_button(label="Play AI", width=380, callback=self._select_ai_mode)
            dpg.add_button(label="Human via Sound - I am White / Host", width=380, callback=self._select_sound_white)
            dpg.add_button(label="Human via Sound - I am Black / Join", width=380, callback=self._select_sound_black)
            dpg.add_spacer(height=6)
            dpg.add_text("Sound mode needs ggwave and pyaudio installed.")
